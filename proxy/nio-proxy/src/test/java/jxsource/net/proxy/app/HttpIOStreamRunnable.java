...
public class HttpIOStreamRunnable extends ChannelRunnable {
	...
	public HttpIOStreamRunnable(ChannelTestManager mgr, String[] urls) {
		super(mgr);
		logger = Logger.getLogger(HttpIOStreamRunnable.class);
		this.urls = urls;
		try {
			...
			while (ready) {
				...
				while (selectionKeys.hasNext()) {
					...
					if (key.isConnectable()) {
						...
						if(length > 0) {
							FileOutputStream out = new FileOutputStream("c:/temp/x.mp4");
...
*/
	public long procLength(long length, SocketChannel from,
			OutputStream out) throws IOException {
		synchronized(from.blockingLock()) {
			from.configureBlocking(true);
