...
public class HttpProxyClientApp implements Runnable{
...
//			trans("http://localhost:10090/LargeChunked");
			trans("http://localhost:10090/HeaderOnly");
//			System.out.println(trans("http://localhost"));

//			System.out.println(trans("http://localhost:10090/Close"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private Socket getSocket() throws IOException {
		if(socket == null) {
		SocketAddress addr = new InetSocketAddress("localhost", 10090);
		socket = new Socket();
		socket.connect(addr);
		}
		return socket;
	...
	public byte[] getResult() throws IOException, HttpException {
		...
		if(headerUtils.hasHeaderWithValue("Connection", "close")) {
			byte[] buf = getOutput(in, 99999999);
			return buf;
		} else {
